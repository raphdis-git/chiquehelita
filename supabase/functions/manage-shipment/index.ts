...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const from: Record<string, unknown> = { name: settings.sender_name, email: settings.sender_email, phone: digits(settings.sender_phone), address: settings.sender_address, complement: settings.sender_address_complement || "", number: settings.sender_address_number, district: settings.sender_district, city: settings.sender_city, postal_code: digits(settings.origin_postal_code), state_abbr: String(settings.sender_state).toUpperCase(), state_register: settings.sender_state_register || "ISENTO" };
    from[senderTax.length === 14 ? "company_document" : "document"] = senderTax;
    const to: Record<string, unknown> = { name: order.customer_name, phone: digits(order.customer_phone), address: order.address, complement: "", number: order.address_number, district: order.district, city: order.city, postal_code: digits(order.postal_code), state_abbr: String(order.state).toUpperCase(), country_id: "BR" };
