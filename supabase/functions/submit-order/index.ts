...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: order, error: orderError } = await client.from("orders").insert({ customer_name:text(customer.name), customer_email:text(customer.email), customer_tax_id:taxId, customer_phone:phone, address:text(customer.address), address_number:text(customer.addressNumber), district:text(customer.district), city:text(customer.city), state:text(customer.state,2).toUpperCase(), postal_code:postalCode, fulfillment:customer.fulfillment, payment_method:text(customer.payment), notes:text(customer.notes,1000)||null, total_quantity:totalQuantity, total_amount:totalAmount }).select("id,order_number").single();
