...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const general = await client.from("store_settings").select("minimum_wholesale_quantity,infinitepay_enabled,infinitepay_test_mode").limit(1).single();
