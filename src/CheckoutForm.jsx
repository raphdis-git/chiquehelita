...
export default function CheckoutForm({ whatsapp, lines, summary, money }) {
  ...
  return <form className="checkout-form" onSubmit={submit} noValidate>
    ...
    <label>E-mail<input type="email" value={customer.email} onChange={(event) => update('email', event.target.value)} autoComplete="email" aria-invalid={Boolean(errors.email)}/>{errors.email && <small>{errors.email}</small>}</label>
    <div className="checkout-location checkout-documents">
      <label>CPF ou CNPJ <span>(somente números)</span><input inputMode="numeric" value={customer.taxId} onChange={(event) => update('taxId', onlyDigits(event.target.value, 14))} autoComplete="off" aria-invalid={Boolean(errors.taxId)}/>{errors.taxId && <small>{errors.taxId}</small>}</label>
      <label>Telefone <span>(somente números)</span><input type="tel" inputMode="numeric" value={customer.phone} onChange={(event) => update('phone', onlyDigits(event.target.value, 11))} autoComplete="tel" aria-invalid={Boolean(errors.phone)}/>{errors.phone && <small>{errors.phone}</small>}</label>
    </div>
    <label>Endereço (rua/avenida)<input value={customer.address} onChange={(event) => update('address', event.target.value)} autoComplete="address-line1" aria-invalid={Boolean(errors.address)}/>{errors.address && <small>{errors.address}</small>}</label>
    <label>Quadra / lote / número<input value={customer.addressNumber} onChange={(event) => update('addressNumber', event.target.value)} autoComplete="address-line2" placeholder="Caso não tenha número, informe S/N" aria-invalid={Boolean(errors.addressNumber)}/>{errors.addressNumber && <small>{errors.addressNumber}</small>}</label>
    <label>Bairro/setor<input value={customer.district} onChange={(event) => update('district', event.target.value)} aria-invalid={Boolean(errors.district)}/>{errors.district && <small>{errors.district}</small>}</label>
    ...
    <label>CEP <span>(somente números)</span><input inputMode="numeric" value={customer.postalCode} onChange={(event) => update('postalCode', onlyDigits(event.target.value, 8))} autoComplete="postal-code" aria-invalid={Boolean(errors.postalCode)}/>{errors.postalCode && <small>{errors.postalCode}</small>}</label>
