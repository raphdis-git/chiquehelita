...
export default function AdminApp() {
  ...
  const [shippingDraft, setShippingDraft] = useState({ originPostalCode:'', weightGrams:'500', packagingTareGrams:'100', heightCm:'10', widthCm:'20', lengthCm:'30', maxItems:'5', handlingDays:'1', markup:'0', melhorEnvioEnabled:false, correiosEnabled:false, localDeliveryEnabled:false, localDeliveryPrice:'0', localDeliveryDays:'1', localDeliveryCity:'', localDeliveryState:'' });
  ...
  async function loadDashboard() {
    ...
    setShippingDraft({
      ...
      localDeliveryEnabled:Boolean(settingsResult.data?.local_delivery_enabled), localDeliveryPrice:String(settingsResult.data?.local_delivery_price ?? 0), localDeliveryDays:String(settingsResult.data?.local_delivery_days ?? 1), localDeliveryCity:settingsResult.data?.local_delivery_city ?? '', localDeliveryState:settingsResult.data?.local_delivery_state ?? '',
    });
  ...
  async function handleSaveSettings(event) {
    ...
    const localDelivery = { local_delivery_enabled:shippingDraft.localDeliveryEnabled, local_delivery_price:Number(shippingDraft.localDeliveryPrice), local_delivery_days:Number(shippingDraft.localDeliveryDays), local_delivery_city:shippingDraft.localDeliveryCity.trim(), local_delivery_state:shippingDraft.localDeliveryState.trim().toUpperCase() };
    setSettingsMessage('');
    ...
    if (localDelivery.local_delivery_enabled && (!localDelivery.local_delivery_city || !/^[A-Z]{2}$/.test(localDelivery.local_delivery_state) || !Number.isFinite(localDelivery.local_delivery_price) || localDelivery.local_delivery_price < 0 || !Number.isInteger(localDelivery.local_delivery_days) || localDelivery.local_delivery_days < 1)) { setSettingsMessage('Revise a cidade, UF, valor e prazo da entrega local.'); return; }
          ...
          {adminSection === 'settings' && <section className="admin-panel admin-settings-panel">
            ...
            <form className="admin-settings-form" onSubmit={handleSaveSettings}>
              ...
              <section className="admin-shipping-settings"><header><div><p className="admin-eyebrow">FRETE AUTOMÁTICO</p><h3>Origem e valores padrão</h3><span>Estes dados serão usados somente quando um produto ainda não possuir peso e dimensões próprias.</span></div></header>
                ...
                <section className="admin-sender-settings admin-local-delivery-settings"><header><div><p className="admin-eyebrow">ENTREGA PRÓPRIA</p><h3>Entrega local / motoboy</h3><span>Ofereça um valor fixo somente para clientes da cidade configurada.</span></div><label className="admin-local-delivery-toggle"><input type="checkbox" checked={shippingDraft.localDeliveryEnabled} onChange={(e) => setShippingDraft((current) => ({...current,localDeliveryEnabled:e.target.checked}))}/><strong>{shippingDraft.localDeliveryEnabled ? 'Ativada' : 'Desativada'}</strong></label></header><div className="admin-settings-grid sender"><label>Cidade atendida<input value={shippingDraft.localDeliveryCity} onChange={(e) => setShippingDraft((current) => ({...current,localDeliveryCity:e.target.value}))} placeholder="Goiânia" disabled={!shippingDraft.localDeliveryEnabled}/></label><label>Estado (UF)<input value={shippingDraft.localDeliveryState} maxLength="2" onChange={(e) => setShippingDraft((current) => ({...current,localDeliveryState:e.target.value.replace(/[^a-z]/gi,'').toUpperCase().slice(0,2)}))} placeholder="GO" disabled={!shippingDraft.localDeliveryEnabled}/></label><label>Valor da entrega (R$)<input type="number" min="0" step="0.01" value={shippingDraft.localDeliveryPrice} onChange={(e) => setShippingDraft((current) => ({...current,localDeliveryPrice:e.target.value}))} disabled={!shippingDraft.localDeliveryEnabled}/></label><label>Prazo estimado (dias úteis)<input type="number" min="1" step="1" value={shippingDraft.localDeliveryDays} onChange={(e) => setShippingDraft((current) => ({...current,localDeliveryDays:e.target.value}))} disabled={!shippingDraft.localDeliveryEnabled}/></label></div></section>
