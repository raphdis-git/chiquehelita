...
export default function AdminApp() {
  ...
  const [expandedClientTaxId, setExpandedClientTaxId] = useState('');
      ...
      <div className="admin-main-area">
        ...
        <div className="admin-content">
          ...
          {adminSection === 'clients' && <section className="admin-panel">
            ...
            {clients.length === 0 ? <div className="admin-empty-state">Nenhum cliente corresponde à busca.</div> : <div className="admin-client-table-wrap"><table className="admin-client-table"><thead><tr><th>Cliente</th><th>Contato</th><th>Detalhes</th></tr></thead><tbody>{clients.map((client) => { const expanded = expandedClientTaxId === client.taxId; const toggleClient = () => setExpandedClientTaxId(expanded ? '' : client.taxId); return <React.Fragment key={client.taxId}><tr className={expanded ? 'admin-client-row expanded' : 'admin-client-row'} tabIndex="0" aria-expanded={expanded} onClick={toggleClient} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); toggleClient(); } }}><td data-label="Cliente"><div className="admin-client-identity"><div className="admin-client-avatar">{client.name.split(/\s+/).slice(0,2).map((part) => part[0]).join('').toUpperCase()}</div><div><strong>{client.name}</strong><span>{client.city}/{client.state}</span></div></div></td><td data-label="Contato"><div className="admin-client-contact"><span>{client.email || 'E-mail não informado'}</span><span>{client.phone}</span><small>CPF/CNPJ: {client.taxId}</small></div></td><td data-label="Detalhes"><span className="admin-client-toggle">{expanded ? 'Ocultar valores' : 'Ver valores'}<ChevronDown size={16}/></span></td></tr>{expanded && <tr className="admin-client-detail-row"><td colSpan="3"><div className="admin-client-detail"><div><span>Total de pedidos</span><strong className="admin-client-orders">{client.orders}</strong></div><div><span>Valor aprovado</span><strong className="admin-client-approved">{money(client.approvedTotal)}</strong></div><div><span>Valor cancelado</span><strong className="admin-client-cancelled">{money(client.cancelledTotal)}</strong></div><div><span>Último pedido</span><strong className="admin-client-date">{new Date(client.lastOrder).toLocaleDateString('pt-BR')}</strong></div></div></td></tr>}</React.Fragment>; })}</tbody></table></div>}
          </section>}
