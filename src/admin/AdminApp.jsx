      ...
      <div className="admin-main-area">
        ...
        <div className="admin-content">
          ...
          {adminSection === 'clients' && <section className="admin-panel">
            ...
            {clients.length === 0 ? <div className="admin-empty-state">Nenhum cliente corresponde à busca.</div> : <div className="admin-client-table-wrap"><table className="admin-client-table"><thead><tr><th>Cliente</th><th>Contato</th><th>Pedidos</th><th>Valor aprovado</th><th>Valor cancelado</th><th>Último pedido</th></tr></thead><tbody>{clients.map((client) => <tr key={client.taxId}><td data-label="Cliente"><div className="admin-client-identity"><div className="admin-client-avatar">{client.name.split(/\s+/).slice(0,2).map((part) => part[0]).join('').toUpperCase()}</div><div><strong>{client.name}</strong><span>{client.city}/{client.state}</span></div></div></td><td data-label="Contato"><div className="admin-client-contact"><span>{client.email || 'E-mail não informado'}</span><span>{client.phone}</span><small>CPF/CNPJ: {client.taxId}</small></div></td><td data-label="Pedidos"><strong className="admin-client-orders">{client.orders}</strong></td><td data-label="Valor aprovado"><strong className="admin-client-approved">{money(client.approvedTotal)}</strong></td><td data-label="Valor cancelado"><strong className="admin-client-cancelled">{money(client.cancelledTotal)}</strong></td><td data-label="Último pedido"><span className="admin-client-date">{new Date(client.lastOrder).toLocaleDateString('pt-BR')}</span></td></tr>)}</tbody></table></div>}
          </section>}
