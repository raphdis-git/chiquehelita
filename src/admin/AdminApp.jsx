...
const MELHOR_ENVIO_STATUSES = { pending: 'Pendente', released: 'Liberado', generated: 'Etiqueta gerada', received: 'Recebido no ponto de distribuição', posted: 'Postado', delivered: 'Entregue', cancelled: 'Cancelado', undelivered: 'Entrega não realizada', paused: 'Entrega pausada', suspended: 'Suspensão de entrega solicitada' };
        ...
        <div className="admin-content">
          ...
          {adminSection === 'orders' && <section className="admin-panel">
            ...
            {dashboardLoading ? <div className="admin-inline-loading"><div className="admin-spinner"/><span>Atualizando pedidos...</span></div> : orders.length === 0 ? <div className="admin-empty-state">Nenhum pedido recebido até o momento.</div> : filteredOrders.length === 0 ? <div className="admin-empty-state">Nenhum pedido corresponde aos filtros.</div> : <div className="admin-order-list">{filteredOrders.map((order) => {
              ...
              const trackingLabel = automaticTracking ? (MELHOR_ENVIO_STATUSES[order.shipping_label_status] ?? TRACKING_STATUSES[order.tracking_status ?? 'awaiting_shipment']) : TRACKING_STATUSES[order.tracking_status ?? 'awaiting_shipment'];
              return <article className={`admin-order-card ${expanded ? 'expanded' : ''}`} key={order.id}>
                ...
                {expanded && <div className="admin-order-details" id={detailsId}>
                  ...
                  {order.fulfillment === 'delivery' && <section className="admin-order-tracking"><div className="admin-order-tracking-heading"><div><strong>Acompanhamento da entrega</strong><span>{automaticTracking ? 'Atualização automática vinculada ao Melhor Envio. Estes dados são somente para consulta.' : 'Preencha os dados para acompanhar esta entrega manualmente.'}</span></div><span className={`tracking-status tracking-${order.tracking_status ?? 'awaiting_shipment'}`}>{trackingLabel}</span></div>{automaticTracking && <div className="admin-shipment-flow"><div><strong>Etiqueta do Melhor Envio</strong><span>{order.shipping_generated_at ? 'Etiqueta pronta para impressão.' : order.shipping_external_id ? 'No carrinho — aguardando compra e geração.' : order.inventory_committed_at ? 'Pedido confirmado — pronto para preparar.' : 'Confirme o pedido para reservar o estoque e liberar a etiqueta.'}</span></div><div>{order.shipping_label_url ? <a className="admin-secondary-button" href={order.shipping_label_url} target="_blank" rel="noreferrer">Abrir etiqueta</a> : order.shipping_external_id ? <button type="button" className="admin-primary-button" disabled={orderSaving === order.id} onClick={() => processShipment(order, 'purchase')}>{orderSaving === order.id ? 'Processando...' : 'Comprar e gerar etiqueta'}</button> : <button type="button" className="admin-primary-button" disabled={orderSaving === order.id || !order.inventory_committed_at || order.status === 'cancelled'} onClick={() => processShipment(order, 'prepare')}>{orderSaving === order.id ? 'Preparando...' : 'Adicionar ao carrinho de envios'}</button>}</div></div>}<div className="admin-order-tracking-fields"><label>Status da entrega<select value={trackingDrafts[order.id]?.status ?? 'awaiting_shipment'} disabled={automaticTracking} onChange={(event) => setTrackingDrafts((current) => ({ ...current, [order.id]: { ...current[order.id], status: event.target.value } }))}>{Object.entries(TRACKING_STATUSES).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Código de rastreio<input value={trackingDrafts[order.id]?.code ?? ''} readOnly={automaticTracking} onChange={(event) => setTrackingDrafts((current) => ({ ...current, [order.id]: { ...current[order.id], code: event.target.value } }))} placeholder={automaticTracking ? 'Preenchido automaticamente' : 'Ex.: AA123456789BR'} maxLength="100"/></label><label className="tracking-url-field">Link de acompanhamento<input type="url" value={trackingDrafts[order.id]?.url ?? ''} readOnly={automaticTracking} onChange={(event) => setTrackingDrafts((current) => ({ ...current, [order.id]: { ...current[order.id], url: event.target.value } }))} placeholder={automaticTracking ? 'Preenchido automaticamente' : 'https://...'} maxLength="500"/></label><label className="tracking-external-id">ID da etiqueta no Melhor Envio<input value={trackingDrafts[order.id]?.externalId ?? ''} readOnly={automaticTracking} onChange={(event) => setTrackingDrafts((current) => ({ ...current, [order.id]: { ...current[order.id], externalId: event.target.value } }))} placeholder="Preenchido automaticamente ao preparar a etiqueta" maxLength="100"/></label></div><div className="admin-order-tracking-actions">{automaticTracking && <span className="admin-automatic-tracking-note">Atualizado automaticamente pelo Melhor Envio</span>}{order.shipping_label_status && <span>Último evento: <strong>{MELHOR_ENVIO_STATUSES[order.shipping_label_status] ?? order.shipping_label_status}</strong></span>}{order.tracking_url && <a href={order.tracking_url} target="_blank" rel="noreferrer">Abrir rastreamento</a>}{automaticTracking && order.shipping_external_id && <button type="button" className="admin-secondary-button" disabled={orderSaving === order.id} onClick={() => syncShipmentStatus(order)}>{orderSaving === order.id ? 'Sincronizando...' : 'Sincronizar agora'}</button>}{!automaticTracking && <button type="button" className="admin-primary-button" disabled={orderSaving === order.id} onClick={() => saveOrderTracking(order)}><Save size={15}/>{orderSaving === order.id ? 'Salvando...' : 'Salvar rastreio'}</button>}</div></section>}
                  ...
                  <footer className="admin-order-status-control"><div><strong>Status do pedido</strong><span>Controle interno do atendimento e pagamento · {order.total_quantity} peças. O andamento da entrega pelo Melhor Envio é atualizado separadamente.</span></div><label><span>Situação atual</span><select value={order.status} disabled={orderSaving === order.id} onChange={(event) => updateOrderStatus(order.id, event.target.value)}>{Object.entries(ORDER_STATUSES).map(([value,label]) => <option value={value} key={value}>{label}</option>)}</select></label></footer>
                </div>}
              </article>;
            })}</div>}
          </section>}
