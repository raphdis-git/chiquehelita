...
import { getAvailableProducts, getProductPrice, getRetailPrice, getTotalStock, isWholesaleCart, storeSettings } from './data/products';
import './styles.css';
...
function App() {
  ...
  const wholesaleActive = isWholesaleCart(totalQuantity);
  const retailPrice = getRetailPrice(product);
  const currentPrice = getProductPrice(product, totalQuantity);
  const subtotal = totalQuantity * currentPrice;
  const totalStock = getTotalStock(product);
  const piecesToWholesale = Math.max(0, storeSettings.minimumWholesaleQuantity - totalQuantity);
  ...
  return (
    <div className="app">
      ...
      {cartOpen && <div className="overlay" onClick={() => setCartOpen(false)}><aside className="cart" onClick={(event) => event.stopPropagation()}><div className="cart-head"><h2>Seu carrinho</h2><button className="icon" onClick={() => setCartOpen(false)} aria-label="Fechar"><X/></button></div>{totalQuantity === 0 ? <p className="empty">Seu carrinho está vazio.</p> : <>{cartLines.map((item) => <div className="cart-item" key={item.label}><img src={product.image} alt={product.name}/><div><strong>{product.name}</strong><span>Tamanho {item.label}</span><span>Estoque: {item.stock}</span><b>{money(currentPrice)} por peça</b><div className="stepper"><button onClick={() => changeQuantity(item.label, -1)}><Minus size={15}/></button><span>{cart[item.label]}</span><button disabled={cart[item.label] >= item.stock} onClick={() => changeQuantity(item.label, 1)}><Plus size={15}/></button></div></div></div>)}<div className={`wholesale-cart-status ${wholesaleActive ? 'active' : ''}`}>{wholesaleActive ? `Atacado ativado para as ${totalQuantity} peças do pedido.` : `Adicione mais ${piecesToWholesale} ${piecesToWholesale === 1 ? 'peça' : 'peças'} para liberar o atacado.`}</div><div className="cart-total"><span>Total {wholesaleActive ? '(atacado)' : '(varejo)'} · {totalQuantity} peças</span><strong>{money(subtotal)}</strong></div><a className="button full" href={`https://wa.me/${storeSettings.whatsapp}?text=${encodeURIComponent(`Olá! Quero fazer este pedido: ${orderDetails}. Total de peças: ${totalQuantity}. Modalidade: ${wholesaleActive ? 'ATACADO' : 'VAREJO'}. Valor unitário atual: ${money(currentPrice)}. Total: ${money(subtotal)}.`)}`} target="_blank" rel="noreferrer"><MessageCircle size={18}/> Finalizar pelo WhatsApp</a></>}</aside></div>}
    </div>
  );
}
